...
# NEW: helper function for better error handling
def decode_revert_reason(error: ContractLogicError) -> str:
    """Extract Vyper assert message from raw revert data"""
    revert_data = error.revert_msg  # Raw bytes from Ganache
    
    if isinstance(revert_data, dict):
        revert_data = revert_data.get('data', b'')  # Geth format
    
    if isinstance(revert_data, str):
        revert_data = bytes.fromhex(revert_data[2:]) if revert_data.startswith('0x') else revert_data.encode()
    
    # Vyper assert: 0x08c379a0 + Error(string) selector + padded string
    if revert_data.startswith(b'\x08\xc3\x79\xa0'):
        try:
            # Skip 4-byte selector, decode dynamic string (offset + length + data)
            offset = int.from_bytes(revert_data[4:36], 'big')
            length = int.from_bytes(revert_data[offset:offset+32], 'big')
            message = revert_data[offset+32:offset+32+length].decode('utf-8')
            return f"🛑 VYPER ASSERT: '{message}'"
        except:
            pass
    
    return f"generic revert: {error}"
...
def run_deposit():
    try:
        ...
        print(f"❌ PRE-SIM FAIL deposit.")
        state_dict = get_state()
        print_state(state_dict)
        print(decode_revert_reason(ex))  # NEW ← Replaces generic message
    except Exception as e:
        state_dict = get_state()
        print_state(state_dict)
...
def add_conditions(description):
    ...
    try:
        ...
        print(f"❌ PRE-SIM FAIL add conditions.")
        state_dict = get_state()
        print_state(state_dict)
        print(decode_revert_reason(ex)) # NEW
        return
    except Exception as ex:
...
def fulfill_conditions(indices):
    ...
    for idx2 in unique_indices:
        ...
        try:
            ...
            print(f"❌ PRE-SIM FAIL fulfill conditions.")
            state_dict = get_state()
            print_state(state_dict)
            print(decode_revert_reason(ex)) # NEW
            continue  
    ...
    try:
       ...
       # Handle mismatch
        ...
        try:
            ...
            print(f"❌ PRE-SIM FAIL deposit to verifier.")
            state_dict = get_state()
            print_state(state_dict)
            print(decode_revert_reason(ex))  # NEW
            return  
...
def run_incomplete_and_refund():
    ...
    try:
        ...
        error_msg = decode_revert_reason(ex)  # ← Gets exact Vyper assert message
        print(f"❌ Refund PRE-SIM FAILED: {error_msg}")
        
        # Show current contract state for debugging
        current_state = get_state()
        print_state(current_state)  # Use your existing print_state helper
        
        # Show ALL conditions status (not just fulfilled ones)
        print("\n📋 ALL CONDITIONS:")
        num_conditions = escrow.functions.get_num_conditions().call()
        for i in range(num_conditions):
            desc, is_fulfilled = escrow.functions.get_condition(i).call()
            status = "✅" if is_fulfilled else "❌"
            print(f"  {status} Condition {i}: '{desc}'")
        
        # Check external condition if applicable
        try:
            external_ok = condition_verifier.functions.verify_condition_for_parties(
                condition_id, escrow.functions.buyer().call(), beneficiary
            ).call()
            print(f"🔗 External condition: {'✅ OK' if external_ok else '❌ FAILED'}")
        except:
            print("🔗 External condition: N/A")
        
        print(f"\n💡 Refund requires: state=1 + timeout passed + NOT all conditions fulfilled")
    ...
    if not tests_to_run:
        ...
        for test in tests_to_run:
            ...
            # Summaries and Audits
                ...
                print("  get_condition_details, escrow_summary, full_audit")

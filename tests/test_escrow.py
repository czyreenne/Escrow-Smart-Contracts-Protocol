...
def setup_contract(timeout): 
    ...
    contract_address, escrow_abi = deploy_escrow_contract(seller.address, timeout)

    escrow = w3.eth.contract(address=contract_address, abi = escrow_abi)
    return escrow, buyer, buyer_priv, seller, seller_priv
...
def test_edge_timeout():
    ...
    print()
    escrow, buyer, buyer_priv, seller, seller_priv = setup_contract(3600) # Cannot let the fast forward increment
    ...
    try:
        ...
        run_incomplete_and_refund(3601, escrow, buyer, buyer_priv) # This should succeed - note that if we do 3600.00000000001 it fails. 
    except Exception as e:
        print(e)
...
def test_refund_after_completion():
    ...
    try:
        ...
        print("===Fastforwarding and attempting refund by just over 3600===")
        run_incomplete_and_refund(3601, escrow, buyer, buyer_priv) # This should fail, but not because of timeout. 
...
def test_invalid_index():
    escrow, buyer, buyer_priv, seller, seller_priv = setup_contract(3600)
    try:
        print("===Attempting Deposit===") # we need this to test release later in max conditions
        deposit_transaction(escrow,buyer,buyer_priv)
        for i in range(11):
...
def test_zero_conditions():
    ...
    try:
        ...
        deposit_transaction(escrow,buyer,buyer_priv)
        print("===Attempting Release===")
        run_release(escrow,seller,seller_priv) # This actually succeeds although it probably shouldn't
    except Exception as e:
        print(e)
    
    print("Testing zero condition refund")
    escrow, buyer, buyer_priv, seller, seller_priv = setup_contract(3600)
    try:
        print("===Attempting Deposit===")
        deposit_transaction(escrow,buyer,buyer_priv)
        print("===Attempting Release===")
        run_incomplete_and_refund(3601,escrow,buyer,buyer_priv) # This actually fails although it should probably pass
        all_conditions_fulfilled(escrow, seller)
    except Exception as e:
        print(e)
...
def test_empty_description():
    ...
    try:
        ...
        add_condition_empty(escrow,buyer,buyer_priv) # Successfully adds condition with no description, should be allowed.
        print("===Checking Conditions===")
        print_all_conditions(escrow)
    except Exception as e:
        print(e)
